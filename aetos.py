...
""", (
    ...
    def calculate(self):
        if self.var_month.get()=='' or self.var_year.get()=='' or self.var_b_salary.get()=='' or self.var_absents.get()=='' or self.var_t_days.get()=='':
            ...
            per_day=float(self.var_b_salary.get())/float(self.var_t_days.get())
        ...
        try:
            ...
            if op:
                for record in selected_records:
                    ...
                    other_allowance = float(selected_record[11]) if selected_record[11] else 0

                    # Handle deductions
                    pf = float(selected_record[15]) if selected_record[15] else 0
                    professional_tax = float(selected_record[14]) if selected_record[14] else 0
                    esic = float(selected_record[16]) if selected_record[16] else 0
                    ...
                    data = {
                        'employee_name': selected_record_employee[3],
                        'month':selected_record[3],
                        'year':selected_record[4],
                        'basic_salary': selected_record[7],
                        'Designation': selected_record_employee[2],
                        'Department': selected_record_employee[5],
                        'uan_no':selected_record_employee[9],
                        'bank_acc':selected_record_employee[10],
                        'pan_no':selected_record_employee[11],
                        'conveyance':conveyance,
                        'doj': selected_record_employee[7],
                        'DaysPaid': float(selected_record[6]),
                        'DaysPresent': float(selected_record[6]) - float(selected_record[5]),
...
# -------------- employee salary frame ------------------# 
    ...
    def print(self):
        try:
            ...
            # Now perform the calculations:
            addition = float(self.var_b_salary.get())+medical + hra + standard_da + educational_allowance + conveyance + other_allowance
